...
namespace M01 {
  namespace HID {
    ...
    void
    MouseControl::press (Page page, Mouse::Button button) {
      if (page != MOUSE)
        return;
    ...
    void
    MouseControl::release (Page page, Mouse::Button button) {
      if (page != MOUSE)
        return;
    ...
    void
    MouseControl::move (int8_t x, int8_t y) {
      if (x != 0 || y != 0) {
    ...
    void
    MouseControl::warp (uint8_t warp_cmd) {
      if (is_warping == false) {
        begin_warping ();
      }

      if (warp_cmd & Mouse::WARP_END) {
        end_warping ();
        return;
      }
      ...
      if (warp_cmd & Mouse::WARP_UP) {
      } else if (warp_cmd & Mouse::WARP_DOWN) {
        section_top  = section_top + next_height;
      }

      if (warp_cmd & Mouse::WARP_LEFT) {
      } else if (warp_cmd & Mouse::WARP_RIGHT) {
    ...
    void
    MouseControl::warp_jump (long left, long top, long height, long width) {
    ...
    void
    MouseControl::end_warping () {
      is_warping= false;
    }

    double
    MouseControl::acceleration (double cycles) {
